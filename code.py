...
def categorical(df):
    ...
    """
    categorical_var = weather.select_dtypes(include='object')
...
def numerical(df):
    ...
    """
    numerical_var = weather.select_dtypes(include='number')
...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    aggregate_value = pd.pivot_table(df,index = 'Month',values=agg_col,aggfunc=agg)
